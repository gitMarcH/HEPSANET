...
### Model with different sensitivity / specificity per site (fixed effects)
...
### Model with different sensitivity / specificity per site (random effects)
...
jagsModel1_te115_apri20 <- jags.model('jagsModel1_NoIndivRandomNoCovariates.jags', data=datJags, n.chains = 4, n.adapt = 10000)
...
```{r model4NoBetweenStudyEffects_apri20_te115, warning=FALSE, message=FALSE, error=FALSE, results='hide'}
datJags<-list()
...
jagsModel4_te115_apri20 <- jags.model('jagsModel4_IndivAndStudyCovariatesNoBetweenStudyEffects_withAlc.jags', data=datJags, n.chains = 4, n.adapt = 5000)
...
We also try a simplified version of the model above where we only include a term for testing reason `L` (suspected liver disease).
...
jagsModel4_te115_apri20 <- jags.model('jagsModel4_IndivAndStudyCovariatesNoBetweenStudyEffects_withAlc_simplerTestReason.jags', data=datJags, n.chains = 4, n.adapt = 2000)
parsModel4_te115_apri20<-coda.samples(model=jagsModel4_te115_apri20,variable.names=c('sens','spec','orGammaL','orGammaA','orLambda','reSens','reSpec'),n.iter=5000)
dicModel4_te115_apri20<-dic.samples(jagsModel4_te115_apri20,type="pD",n.iter=1000) 

#print(summary(parsModel4_te115_apri20))
#print(gelman.diag(parsModel4_te115_apri20))

# pdf(width=10,height=10,file=paste(sep="/",outDir,"HEPSANET_jagsModel4_te115_apri20_IndivAndStudyCovariatesNoBetweenStudyEffects_withAlc_simplerTestReason.pdf"))
# plot(parsModel4_te115_apri20,auto.layout=FALSE)
# dev.off()
#This model has a DIC of `r sum(dicModel4_te115_apri20$deviance)+sum(dicModel4_te115_apri20$penalty)` and the multivariate Gelman-Rubin convergence diagnostic statistics is `r gelman.diag(parsModel4_te115_apri20)$mpsrf`.
```
...
```{r model4_apri20_te115SimpleSummary}
tmp<-try(summary(parsModel4_te115_apri20)$statistics,silent=TRUE)
tmpCI<-t(hdi(parsModel4_te115_apri20))
tmp<-cbind(tmp,tmpCI)
...
## APRI ROC curve for te threshold 11.5

```{r apriVect, warning=F, message=F, error=FALSE, results='hide'}
#apriVect<-c(0,seq(0.1,0.7,by=0.05),seq(0.7,3,by=0.1))
apriVect<-unique(c(0,1.5,2,quantile(dat$apri,probs=seq(0,1,length=40))))
apriVect<-sort(apriVect)

apriResultsList<-list()
apriModList<-list()
...
for(i in 1:length(apriVect)){
  ...
  jagsModel4_te115_apri <- jags.model('jagsModel4_IndivAndStudyCovariatesNoBetweenStudyEffects_withAlc_simplerTestReason.jags', data=datJags, n.chains = 4, n.adapt = 3000)
  parsModel4_te115_apri<-coda.samples(model=jagsModel4_te115_apri,variable.names=c('sens','spec','orGammaL','orGammaA','orLambda','reSens','reSpec'),n.iter=10000)
  
  tmp<-try(summary(parsModel4_te115_apri)$statistics,silent=TRUE)
  tmpCI<-t(hdi(parsModel4_te115_apri))
  tmp<-cbind(tmp,tmpCI)
  ...
  apriModList[[paste(sep="","apri_mod_",apr)]]<-jagsModel4_te115_apri
  apriModList[[paste(sep="","apri_samples_",apr)]]<-parsModel4_te115_apri
  apriResultsList[[paste(sep="","apri",apr)]]<-tmp
}

save(apriResultsList,apriModList,file=paste(sep="","apriResultsList_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
```

```{r apriVectSummary, warning=F, message=F, error=FALSE, results='hide'}
sensVect<-numeric(length(apriVect))
specVect<-numeric(length(apriVect))
for(i in 1:length(apriVect)){
  sensVect[i]<-apriResultsList[[i]]["overall sensitivity","Mean"]
  specVect[i]<-apriResultsList[[i]]["overall specificity","Mean"]
}
...
```{r apriPlot, fig.width=10, fig.height=10}
dfROC %>%
...
```{r model4NoBetweenStudyEffects_apri20_te95, warning=F, message=F, error=FALSE, results='hide'}
datJags<-list()
...
jagsModel4_te95_apri20 <- jags.model('jagsModel4_IndivAndStudyCovariatesNoBetweenStudyEffects_withAlc_simplerTestReason.jags', data=datJags, n.chains = 4, n.adapt = 2000)
parsModel4_te95_apri20<-coda.samples(model=jagsModel4_te95_apri20,variable.names=c('sens','spec','orGammaL','orGammaA','orLambda','reSens','reSpec'),n.iter=5000)
...
```{r model4NoBetweenStudyEffects_apri15_te79, warning=F, message=F, error=FALSE, results='hide'}
datJags<-list()
...
jagsModel4_te79_apri15 <- jags.model('jagsModel4_IndivAndStudyCovariatesNoBetweenStudyEffects_withAlc_simplerTestReason.jags', data=datJags, n.chains = 4, n.adapt = 2000)
parsModel4_te79_apri15<-coda.samples(model=jagsModel4_te79_apri15,variable.names=c('sens','spec','orGammaL','orGammaA','orLambda','reSens','reSpec'),n.iter=5000)
...
## ALT with te threshold 11.5

```{r altVect, warning=F, message=F, error=FALSE, results='hide'}
altVect<-unique(quantile(dat$alt,probs=seq(0,1,length=40)))
altVect<-c(0,altVect)
...
for(i in 1:length(altVect)){
  ...
  jagsModel4_te115_alt <- jags.model('jagsModel4_IndivAndStudyCovariatesNoBetweenStudyEffects_withAlc_simplerTestReason.jags', data=datJags, n.chains = 4, n.adapt = 3000)
  parsModel4_te115_alt<-coda.samples(model=jagsModel4_te115_alt,variable.names=c('sens','spec','orGammaL','orGammaA','orLambda','reSens','reSpec'),n.iter=10000)
  
  tmp<-try(summary(parsModel4_te115_alt)$statistics,silent=TRUE)
  tmpCI<-t(hdi(parsModel4_te115_alt))
  tmp<-cbind(tmp,tmpCI)
  ...
  altModList[[paste(sep="","alt_mod_",alt)]]<-jagsModel4_te115_alt
  altModList[[paste(sep="","alt_samples_",alt)]]<-parsModel4_te115_alt
  altResultsList[[paste(sep="","alt",alt)]]<-tmp
}

save(altResultsList,altModList,file=paste(sep="","altResultsList_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
```

```{r altVectSummary, warning=F, message=F, error=FALSE, results='hide'}
sensVect<-numeric(length(altVect))
specVect<-numeric(length(altVect))
for(i in 1:length(altVect)){
  sensVect[i]<-altResultsList[[i]]["overall sensitivity","Mean"]
  specVect[i]<-altResultsList[[i]]["overall specificity","Mean"]
}
youdenJ<-sensVect+specVect-1
...
```{r altPlot, fig.width=10, fig.height=10}
dfROC %>%
  ggplot(mapping=aes(x=1-specificity,y=sensitivity)) +
  geom_smooth(col="orange",fill="orange",method="scam",formula = y ~ s(x, k = 5, bs = "mpi")) +
  geom_point(alpha=1,col="darkgrey") +
  ...
  labs(title=paste(sep="","ROC curve for ALT with diagnosis threshold te > 11.5kPa.\nAUC = ",format(nsmall=2,round(digits=2,auc))),caption="Bayesian bivariate random-effects models fitted for different threshold on ALT (from 1 to 198 according to 40 equally spaced quantiles).\nGiven the Bayesian nature of the data, there is some variability due to MCMC sampling.\nThe ROC curve (orange) is a shape-constrained generalized additive model fitted to the raw estimates (grey).\nAUC is computed from the raw estimates.") +
  theme_light()

altResultsList[[paste(sep="","alt",bestThr)]] %>%
  kable(caption="Best (according to Youden's J) ALT model with site/study specific sensitivities and specificities (random effects).",digits=4,col.names=c("Posterior mean","95% HDI credible interval"),align=c("r","r")) %>%
  kable_styling(full_width = FALSE)
```

## FIB4 with te threshold 11.5

```{r fib4Vect, warning=F, message=F, error=FALSE, results='hide'}
fib4Vect<-unique(quantile(dat$fib4,probs=seq(0,1,length=40)))
fib4Vect<-sort(c(0,fib4Vect,6,10))

fib4ResultsList<-list()
fib4ModList<-list()
...
for(i in 1:length(fib4Vect)){
  ...
  jagsModel4_te115_fib4 <- jags.model('jagsModel4_IndivAndStudyCovariatesNoBetweenStudyEffects_withAlc_simplerTestReason.jags', data=datJags, n.chains = 4, n.adapt = 3000)
  parsModel4_te115_fib4<-coda.samples(model=jagsModel4_te115_fib4,variable.names=c('sens','spec','orGammaL','orGammaA','orLambda','reSens','reSpec'),n.iter=10000)
  
  tmp<-try(summary(parsModel4_te115_fib4)$statistics,silent=TRUE)
  tmpCI<-t(hdi(parsModel4_te115_fib4))
  tmp<-cbind(tmp,tmpCI)
  ...
  fib4ModList[[paste(sep="","fib4_mod_",fib4)]]<-jagsModel4_te115_fib4
  fib4ModList[[paste(sep="","fib4_samples_",fib4)]]<-parsModel4_te115_fib4
  fib4ResultsList[[paste(sep="","fib4",fib4)]]<-tmp
}

save(fib4ResultsList,fib4ModList,file=paste(sep="","fib4ResultsList_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
```

```{r fib4VectSummary, warning=F, message=F, error=FALSE, results='hide'}
sensVect<-numeric(length(fib4Vect))
specVect<-numeric(length(fib4Vect))
for(i in 1:length(fib4Vect)){
  sensVect[i]<-fib4ResultsList[[i]]["overall sensitivity","Mean"]
  specVect[i]<-fib4ResultsList[[i]]["overall specificity","Mean"]
}
youdenJ<-sensVect+specVect-1
...
```{r fib4Plot, fig.width=10, fig.height=10}
dfROC %>%
  ggplot(mapping=aes(x=1-specificity,y=sensitivity)) +
  geom_smooth(col="orange",fill="orange",method="scam",formula = y ~ s(x, k = 5, bs = "mpi")) +
  geom_point(alpha=1,col="darkgrey") +
  ...
  labs(title=paste(sep="","ROC curve for FIB4 with diagnosis threshold te > 11.5kPa.\nAUC = ",format(nsmall=2,round(digits=2,auc))),caption="Bayesian bivariate random-effects models fitted for different threshold on FIB4 (from 0 to 38.35 according to 40 equally spaced quantiles).\nGiven the Bayesian nature of the data, there is some variability due to MCMC sampling.\nThe ROC curve (orange) is a shape-constrained generalized additive model fitted to the raw estimates (grey).\nAUC is computed from the raw estimates.") +
  theme_light()

fib4ResultsList[[paste(sep="","fib4",bestThr)]] %>%
  kable(caption="Best (according to Youden's J) FIB4 model with site/study specific sensitivities and specificities (random effects).",digits=4,col.names=c("Posterior mean","95% HDI credible interval"),align=c("r","r")) %>%
  kable_styling(full_width = FALSE)
```
